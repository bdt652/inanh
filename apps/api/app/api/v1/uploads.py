...
from app.core.storage import StorageUnavailableError, _build_minio_client, _is_local_backend, store_object_bytes
...
async def direct_upload(
    ...
    try:
        store_object_bytes(key, data, content_type)
    except StorageUnavailableError as exc:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(exc)) from exc

    return UploadDirectResponse(key=key, size=len(data), content_type=content_type)
...
async def complete_batch(
    ...
    total_keys = session["counts"]["total_keys"]
    valid_images = session["counts"]["valid_images"]
    rejected = session["counts"]["rejected"]
    total_bytes = session["counts"]["total_bytes"]
